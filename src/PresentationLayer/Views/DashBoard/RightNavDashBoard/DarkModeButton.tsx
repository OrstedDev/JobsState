...
export const DarkModeButton = () => {
  ...
  useEffect(() => {
    if (state?.initDate !== undefined) {
      let storageTheme: OneValue = new FragmentStorage().GetValueJSON(
        Crypt0.C1pt0ToHex("Theme")
      );

      if (storageTheme?.Value !== undefined) {
        setIsDarkMode(storageTheme.Value);
      }
  ...
  const handleButtonClick = async () => {
    ...
    dispatch({
      type: "UPDATE_STATE",
      payload: { mode: newMode ? "dark" : "light" },
    });

    await new UserUseCase().updateUserDarkMode(newMode);
  ...
  return (
    <FormControlLabel
      control={
        <MaterialUISwitch checked={isDarkMode} onChange={handleButtonClick} />
      }
      label={null}
    />
  );
